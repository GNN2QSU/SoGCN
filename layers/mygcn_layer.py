...
class MyGCNLayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, activation, dropout, graph_norm, batch_norm, residual=False, dgl_builtin=False):
        ...
        self.linear_0 = nn.Linear(in_dim, out_dim)
    ...
    def forward(self, g, feature, snorm_n):
        ...
        h = self.linear_0(h_in)

        g = g.local_var()
        g.ndata['h'] = h_in
        g.update_all(msg, reduce)
        # g.apply_nodes(func=self.apply_mod)
        h += self.linear_1(g.ndata['h'])

        g.update_all(msg, reduce)
        h += self.linear_2(g.ndata['h']) # result of graph convolution
